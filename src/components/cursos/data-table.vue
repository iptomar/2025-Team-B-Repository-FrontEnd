...
<template>
  ...
  <div class="flex-1">
    <Input 
      ...
      placeholder="Procurar por curso ou grau..."
  ...
  <div class="flex-1">
    <select
  class="h-[2.7rem] border border-gray-300 rounded px-2 py-1"
...
>
  <option 
    v-for="ano in anosLetivos.slice().reverse()" 
    :key="ano" 
    :value="ano"
  >
    {{ ano }}
        ...
        <TableBody>
          <template v-if="table.getRowModel().rows?.length">
            <TableRow v-for="row in table.getRowModel().rows" :key="row.id"
              ...
              @click="goToCourse(row.original.id)">
    ...
    <div class="bg-white rounded-lg p-6 w-96">
      ...
      <form @submit.prevent="handleSubmit">
        <div class="mb-4">
          ...
          <input v-model="novoCurso.curso" type="text" class="w-full border border-gray-300 rounded px-2 py-1" required />
        </div>
        ...
        <div class="mb-4">
          ...
          <select v-model="novoCurso.instituicao" class="w-full border border-gray-300 rounded px-2 py-1" required>
            ...
            <option v-for="instituicao in instituicoes" :key="instituicao" :value="instituicao">{{ instituicao }}</option>
          </select>
        </div>
        ...
        <div class="mb-4">
          ...
          <input 
            ...
            :value="anosLetivos[anosLetivos.length - 1]" 
            readonly 
          />
        </div>
        ...
        <div class="flex justify-center space-x-2">
          ...
          <button type="button" @click="showModal = false" class="px-4 py-2 text-white bg-gray-400 hover:bg-gray-100 hover:border-gray-400 hover:text-gray-400 rounded">
            Cancelar
          </button>
        </div>
      </form>
