...
const showModalCadeira = ref(false)
const novaCadeira = ref({ nome: '', ano: 1, semestre: 1, ects: 1 })

const handleSubmitCadeira = () => {
  // Enviar para backend
  console.log(novaCadeira.value);
  showModalCadeira.value = false;
};

const goToCadeira = (id: string) => {
...
<template>
  <div class="flex flex-col h-full w-full">
    <div class="flex items-center pb-4 w-full space-x-20">
      ...
      <button @click="showModalCadeira = true"
        ...
        <TableBody>
          <template v-if="table.getRowModel().rows?.length">
            <TableRow v-for="row in table.getRowModel().rows" :key="row.id"
              ...
              @click="goToCadeira(String(row.original.id))">
  ...
  <div v-if="showModalCadeira" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white rounded-lg p-6 w-96">
      <h2 class="text-xl mb-4">Criar Cadeira</h2>
      <form @submit.prevent="handleSubmitCadeira">
        <div class="mb-4">
          <label class="block mb-1">Nome da Cadeira</label>
        ...
        <div class="mb-4">
          ...
          <input v-model="novaCadeira.semestre" type="number" min="1" max="3" @input="limitValue('semestre', 1, 3)"
            class="w-full border border-gray-300 rounded px-2 py-1" required />
          <p>3 - Para cadeiras anuais</p>
        </div>

        <div class="mb-4">
          <label class="block mb-1">ECTS</label>
        ...
        <div class="flex justify-center space-x-2">
          ...
          <button type="button" @click="showModalCadeira = false"
